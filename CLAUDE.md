...
- `ui/widgets.py` — reusable pieces shared by the window, the tabs and the dialogs: `Card`, `soft_shadow`, `platform_icon`, `NameColumnCursor`, plus the three factories `stat_card` / `make_table` / `make_placeholder`. ⚠️ `stat_card` attaches `_value_label` and `_caption_label` to the frame it returns — a **cross-module contract** the tab slots rely on to update text in place. `_caption_label` is `None` when no caption is passed, so any caller that writes to it must pass one.
