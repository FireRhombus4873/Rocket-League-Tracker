...
class MainWindow(QMainWindow):
    ...
    def __init__(self, close_to_tray: bool = True):
        ...
        self.signals.analytics_updated.connect(self._on_analytics_updated)
    ...
    def _build_analytics_tab(self) -> QWidget:
        ...
        snapshot. Nothing here may touch `self.signals` — _build_ui() runs before
        it exists (see __init__)."""
        tab = QWidget()
        outer = QVBoxLayout(tab)
        outer.setContentsMargins(4, 16, 4, 4)
        outer.setSpacing(0)
    ...
    def _build_analytics_overview_row(self) -> QWidget:
        ...
        self._analytics_record_card = self._stat_card(
            "ALL-TIME RECORD", "0W / 0L", TEXT, "no matches yet",
            value_pt=21, width=206)
        self._analytics_winrate_card = self._stat_card(
            "WIN RATE", "—", ACCENT2, "best run — · worst —",
            value_pt=28, width=178)
        self._analytics_duration_card = self._stat_card(
            "AVG DURATION", "—", TEXT, "no timed matches",
            value_pt=28, width=168)
        self._analytics_overtime_card = self._stat_card(
    ...
    def _build_analytics_leaderboards_row(self) -> QWidget:
        ...
        for title, note_text, empty_text in specs:
            ...
            table = self._make_table(["Player", "Played", "W", "L", "Win %"])
    ...
    def _stat_card(self, label: str, value: str, colour: str, caption: str = "",
